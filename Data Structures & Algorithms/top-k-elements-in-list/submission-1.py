class Solution:
    def topKFrequent(self, nums: List[int], k: int) -> List[int]:
        ...
        for i in range(len(freq) - 1, 0, -1):
            for n in freq[i]:
                res.append(n)
                # we are appending the k most frequent elements, so we descend the freq list of lists
                # and append from higher freq to lower the numbers we put in each sublist
                if len(res) == k:
                    return res
